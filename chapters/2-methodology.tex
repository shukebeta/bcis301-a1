...
comparison and contrast are conducted for the findings of both phases to evaluate the agreement between the phases.

If there is some degree of agreement that can contribute to the evaluation tool, an 2-level hierarchical structure mark sheet will be created using the following method.

A 2-level hierarchical Mark Sheet consists of an outer level of the primary themes we used in our entire research; and the inner level of the sub-themes of when the solution is recommended with a new round of qualitative analysis. The solutions are all populated under the sub-themes with normalized weights calculated as follows.
\begin{equation}
