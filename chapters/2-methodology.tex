...
% 总流程
\subsection{Synopsis}
For this study, a qualitative analysis is first carried out, gathering all the possible proposals that the researchers suggest regarding success, failure and risks identified in project management, change management and risk management. To identify success, failure and risks in the sources, a list of excerpts is built and then reworded as possible proposals. Those proposals are evaluated in a quantitative model which results a checklist.

% 列举文章和主题
\subsection{Selection of Literature}
We focus on three themes regarding to the success of an IT project, namely project management, change management and risk management. Also, CSF(critical success factor), CFF(critical failure factor), ERP(enterprise resource planning) are the keywords used in the literature selections. All the sources should be from professional journals and forums published within 10 years from now, around 10 pages of length and referenced by other researchers, which reliability and professionalism can be assured.
